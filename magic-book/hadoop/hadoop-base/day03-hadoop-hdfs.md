...
tags: Hadoop
categories: day03-HDFS-NameNode
---
...
![1582165941866](day03-hadoop-hdfs/1582165941866.png) 

### 1.2 历史

1. [Doug Cutting]() 在做 Lucene 的时候, 需要编写一个爬虫服务, 这个爬虫写的并不顺利, 遇到了一些问题, 诸如: 如何存储大规模的数据, 如何保证集群的可伸缩性, 如何动态容错等
2. 2013年的时候, Google 发布了三篇论文, 被称作为三驾马车, 其中有一篇叫做 GFS, 是描述了 Google 内部的一个叫做 [GFS]() 的分布式大规模文件系统, 具有强大的可伸缩性(可扩展性)和容错性(容忍出现错误)
...
- 采用流式的数据访问方式: 即[一次写入、多次读取(用于离线分析)]()，数据集经常从数据源生成或者拷贝一次，然后在其上做很多分析工作 。
...
![1565054158842](day03-hadoop-hdfs/1565054158842.png)
...
![1561706903642](day03-hadoop-hdfs/1561706903642.png)		
...
- Client 提供一些命令来管理 和访问HDFS，比如启动或者关闭HDFS(访问客户端命令:hdfs dfs -put a.txt /)。

　　**2、NameNode：就是 master，它是一个主管、管理者。**

- 管理 HDFS 的名称空间,对外提供一个统一的访问路径
- 管理数据块（Block）映射信息,告诉客户端,一个文件的每一个切片(block)所在的位置
- 配置副本策略,每个副本存放在那台主机
- 处理客户端读写请求。
...
- 辅助 NameNode，分担其工作量,不能替代namenode。
- 定期合并 fsimage和fsedits，并推送给NameNode。
- 在紧急情况下，可辅助恢复 NameNode。

## 4:NameNode和DataNode

![1561706945153](day03-hadoop-hdfs/1561706945153.png)	

### 4.1 NameNode作用

* NameNode在内存中保存着整个文件系统的[名称]()[空间（文件的名称即唯一路径）]()和文件数据块的[地址映射（block的地址映射）]()
* 整个HDFS可存储的文件数受限于[NameNode的内存大小]() 

 `1、NameNode元数据信息` 
文件名，文件目录结构，文件属性(生成时间，副本数，权限)每个文件的块列表（这些信息都存放在namenode的内存中）。 
...
[如果有一个datanode挂掉，那么namenode就会将副本重新复制，保持原来的副本数量]()
...
3、Data Node 周期性向NameNode汇报心跳信息包

4、Data Node 周期性向NameNode汇报数据块信息[（块缓存是namenode将经常访问的文件通知datanode将此文件存储到datanode的内存中，提高读取效率）]()

5、Data Node 周期性向NameNode汇报block缓存数据块信息
...
[在 Hadoop1 当中, 文件的 block 块默认大小是 64M, hadoop2 当中, 文件的 block 块大小默认是 128M（每个block默认是128MB，如果不够128MB，也称为一个block，所以block是一个逻辑单位，每个block默认是3个副本）, block 块的大小可以通过 hdfs-site.xml 当中的配置文件进行指定]()
...
![1561706988677](day03-hadoop-hdfs/1561706988677.png)	
...
~~~shell
格式  :   hdfs  dfs -lsr URI==hdfs  dfs ls -R URI
...
~~~shell
格式 ： hdfs  dfs  -mkdir <path>
格式 ： hdfs  dfs [-p] -mkdir <paths>
作用 :   以<paths>中的URI作为参数，创建目录。使用-p参数可以递归创建子目录
~~~

```shell
hdfs  dfs  -mkdir /dir1
hdfs  dfs  -mkdir -p /dir2/dir1 	#递归创建文件夹
```
...
~~~shell
hdfs dfs -put  /root/a.txt  /dir1   #将文件复制到hdf
~~~
...
~~~shell
hdfs  dfs -moveFromLocal  /root/install.log  /   #将文件移动（剪切）到hdfs
~~~
...
否则，在回收站可用时，在HDFS Shell 中执行此命令，会将文件暂时放到回收站中（7天之后自动删除）。
~~~

~~~shell
hdfs  dfs  -rm  -r  /dir1	#递归删除一个文件夹
hdfs  dfs  -rm  /a.txt		#删除一个文件
~~~
...
选项将覆盖目标，如果它已经存在。
-p
选项将保留文件属性（时间戳、所有权、许可、ACL、XAttr）（深度拷贝）。
~~~
...
~~~shell
hdfs dfs -chmod 777 /install.log
hdfs dfs -chmod -R 777 /dir1		#对整个目录的文件或者文件夹有效递归执行权限更改
~~~
...
作用: 追加一个或者多个文件到hdfs指定文件中.也可以从命令行读取输入(<localsrc>:表示本地路径的文件,<dst>:表示hdfs的路径).
~~~
...
#### 7.1.2、空间**大小限额**
 ...
 在设置空间配额时，设置的空间至少是block_size * 3大小==128MB*3==384MB
...
安全模式是hadoop的一种[保护机制]()，用于保证集群中的数据块的安全性。[第一种：当集群启动的时候，会首先进入安全模式。当系统处于安全模式时会检查数据块的完整性，第二种：在HDFS工作中，发现BLOCK副本率不达标，则自动进入安全模式。]()
...
![1565078960482](day03-hadoop-hdfs/1565078960482.png)

## 10.HDFS 文件读取过程

![1558574532408](day03-hadoop-hdfs/1558574532408.png)	
...
![1565080197208](day03-hadoop-hdfs/1565080197208.png)
...
* 配置 SecondaryNameNode
  ...
  * 修改 `core-site.xml`, 这一步不做配置保持默认也可以
    ...
    [SecondaryNameNodeky 可以定期的将fsimage文件和edits文件进行合并，合并成一个新的fsimage文件，并且替换原谅fsimage文件，减轻namenode的压力]()
    
    合并的触发条件：
    
    - 1）时间一个小时
    - 2）文件大小，edits文件达到64MB
  ...
  <property>
    ...
    ![1561707242581](day03-hadoop-hdfs/1561707242581.png)		

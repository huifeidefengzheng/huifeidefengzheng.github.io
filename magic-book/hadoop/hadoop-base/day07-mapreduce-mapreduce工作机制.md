---
title: day07-mapreduce-mapreduce工作机制的运行机制详解
date: 2019-08-12 22:21:46
tags: Hadoop
categories: day07-mapreduce-mapreduce工作机制的运行机制详解
---

## 1 .day07-mapreduce-mapreduce工作机制的运行机制详解

### 1.1:MapTask 工作机制

![1561706243507](day07-mapreduce-mapreduce工作机制/1561706243507.png)	

![1561706253827](day07-mapreduce-mapreduce工作机制/1561706253827.png)	
...
![1565571940217](day07-mapreduce-mapreduce工作机制/1565571940217.png)

- 环形缓冲区

![1565572594968](day07-mapreduce-mapreduce工作机制/1565572594968.png)

- 算法:空间复杂度和时间复杂度(O(log2N))
...
4. Mapper 逻辑结束之后, 将 Mapper 的每条结果通过 `context.write` 进行collect数据收集. 在 collect 中, 会先对其进行分区处理，默认使用 **HashPartitioner**
   ...
   - > mapreduce工作机制 提供 `Partitioner` 接口, 它的作用就是根据 `Key` 或 `Value` 及 `Reducer` 的数量来决定当前的这对输出数据最终应该交由哪个 `Reduce task` 处理, 默认对 Key Hash 后再以 Reducer 数量取模. 默认的取模方式只是为了平均 Reducer 的处理能力, 如果用户自己对 Partitioner 有需求, 可以订制并设置到 Job 上
...
6. 当溢写线程启动后, 需要**对这 80MB 空间内的 Key 做排序 (Sort)**. 排序是 mapreduce工作机制 模型默认的行为, 这里的排序也是对序列化的字节做的排序

   - > 如果 Job 设置过 Combiner, 那么现在就是使用 Combiner 的时候了. 将有相同 Key 的 Key/Value 对的 Value 加起来, 减少溢写到磁盘的数据量. Combiner 会优化 mapreduce工作机制 的中间结果, 所以它在整个模型中会多次使用
...
| 配置                                       | 默认值                           | 解释                       |
| ------------------------------------------ | -------------------------------- | -------------------------- |
| `mapreduce工作机制.task.io.sort.mb`        | 100                              | 设置环型缓冲区的内存值大小 |
| `mapreduce工作机制.map.sort.spill.percent` | 0.8                              | 设置溢写的比例             |
| `mapreduce工作机制.cluster.local.dir`      | `${hadoop.tmp.dir}/mapred/local` | 溢写数据目录               |
| `mapreduce工作机制.task.io.sort.factor`    | 10                               | 设置一次合并多少个溢写文件 |

**配置文件为:mapred-site.xml**

![1565574425861](day07-mapreduce-mapreduce工作机制/1565574425861.png)

### 1.2 :ReduceTask 工作机制

![1561706287927](day07-mapreduce-mapreduce工作机制/1561706287927.png)	
...
2. **`Merge(合并)阶段`**。这里的merge如map端的merge动作，只是数组中存放的是不同map端copy来的数值。Copy过来的数据会先放入内存缓冲区中，这里的缓冲区大小要比map端的更为灵活。merge有三种形式：内存到内存；内存到磁盘；磁盘到磁盘。默认情况下第一种形式不启用。当内存中的数据量到达一定阈值，就启动内存到磁盘的merge。与map 端类似，这也是溢写的过程，这个过程中如果你设置有Combiner，也是会启用的，然后在磁盘中生成了众多的溢写文件。第二种merge方式一直在运行，直到没有map端的数据时才结束，然后启动第三种磁盘到磁盘的merge方式生成最终的文件。
3. **`合并排序(归并)`**。把分散的数据合并成一个大的数据后，还会再对合并后的数据排序。
4. **`对排序后的键值对调用reduce方法`**，键相等的键值对调用一次reduce方法，每次调用会产生零个或者多个键值对，最后把这些输出的键值对写入到HDFS文件中。

![1565575429562](day07-mapreduce-mapreduce工作机制/1565575429562.png)

### 1.3:Shuffle 过程

map 阶段处理的数据如何传递给 reduce 阶段，是 mapreduce工作机制 框架中最关键的一个流程，这个流程就叫 shuffle
shuffle: 洗牌、发牌 ——（核心机制：数据分区，排序，分组，规约，合并等过程）

![1561706306005](day07-mapreduce-mapreduce工作机制/1561706306005.png)	

shuffle 是 mapreduce工作机制 的核心，它分布在 mapreduce工作机制 的 map 阶段和 reduce 阶段。一般把从 Map 产生输出开始到 Reduce 取得数据作为输入之前的过程称作 shuffle。
...
6. **`Sort阶段`**：在对数据进行合并的同时，会进行排序操作，由于 MapTask 阶段已经对数据进行了局部的排序，ReduceTask 只需保证 Copy 的数据的最终整体有效性即可。
   Shuffle 中的缓冲区大小会影响到 mapreduce工作机制 程序的执行效率，原则上说，缓冲区越大，磁盘io的次数越少，执行速度就越快
   缓冲区的大小可以通过参数调整,  参数：mapreduce工作机制.task.io.sort.mb  默认100M

## 2. 案例: Reduce 端实现 JOIN

### 2.1. 需求

> 假如数据量巨大，两表的数据是以文件的形式存储在 HDFS 中, 需要用 day07-mapreduce-mapreduce工作机制 程序来实现以下 SQL 查询运算
>
> ```sql
...
通过将关联的条件作为map输出的key，将两表满足join条件的数据并携带数据所来源的文件信息，发往同一个reduce task，在reduce中进行数据的串联(jion)

![1565576192680](day07-mapreduce-mapreduce工作机制/1565576192680.png)
...
public class ReduceJoinMapper extends Mapper<LongWritable,Text,Text,Text> {
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        /*
        product.txt	k1		v1
        			0		pool,小米5,1000,2000
        oders.txt	k1		v1
        			0		1001,20150710,p0001,2
        --------------------------------------------
        			k2		v2
        			p0001	pool,小米5,1000,2000
        ...
        */
        ...
        if(fileName.equals("product.txt")){
            ...
            //context.write(new Text(productId), "001"+value);
        }else{
            //数据来自订单表
            //2:将K1和V1转为K2和V2,写入上下文中
            String[] split = value.toString().split(",");
			...
			context.write(new Text(productId), value);
            //context.write(new Text(productId), "002"+value);
        }
    }
}
    ...
    protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
       //1:遍历集合,获取V3 (first +second)
        ...
        for (Text value : values) {
            ...
            //if(value.toString().startsWith("001")){ 
                first = value.toString();
               //first = value.toString().subString(3);
            }else{
                second = value.toString();
               //second = value.toString().subString(3);;
            }
        }
        //2:将K3和V3写入上下文中
...
public class JobMain extends Configured implements Tool{
	...
	@override
    public int run(String[] args) throws Excetion{
        //1获取job对象
        Job job = Job.getInstance(super.getConf(),"reduce-join");
        //设置job任务
        	//第一步:设置输入和输入路径
        	job.setInputFormatClass(TextInputFormat.class);
        	TextInputFormat.addInputPath(job,new Path("file:///H:\\input\\reduce-join"))
        	//第二步:设置Mapper类和数据类型
            job.setMapperClass(ReduceJoinMapper.class);
        	job.setMapOutputKeyClass(Text.class);
        	//第三步:分区(partitioner)	第四步:排序()	第五步:规约(combiner)		第六步:分组
        	//第七步:设置reducer类和数据类型
        	...
        	TextOutputFormat.setOutputPath(job,new Path("file:///H:\\input\\reduce-join-out"))
        //等待程序结束
...
![1565592194998](day07-mapreduce-mapreduce工作机制/1565592194998.png)
...
public class JobMain  extends Configured implements Tool{
    ...
    public int run(String[] args) throws Exception {
        ...
        //2:设置job对象(将小表放在分布式缓存中)
            //将小表放在分布式缓存中
           // DistributedCache.addCacheFile(new URI("hdfs://node01:8020/cache_file/product.txt"), super.getConf());//早期使用的方法
...
![1565597445885](day07-mapreduce-mapreduce工作机制/1565597445885.png)
...
-----------------------------------
    ...
    */
    ...
    protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        //2:K2就是V3
        //3:将K3和V3写入上下文中
        context.write(new Text(buffer.toString().subString(0,buffer.length()-1)), key);
    }
}
~~~

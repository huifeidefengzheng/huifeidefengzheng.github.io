...
数据仓库会将不同源数据库中的数据汇总到一起,数据仓库中的综合数据不能从原有的数据库系统直接得到。因此在数据进入数据仓库之前，必然要经过统一与整合，这一步是数据仓库建设中最关键、最复杂的一步([ETL:数据清洗]())，要统一源数据中所有矛盾之处，如字段的同名异义、异名同义、单位不统一、字长不一致，等等。
...
![1561705874041](day09-hive讲义/1561705874041.png)	
...
* 数据仓库的规模及其复杂性离不开正确的元数据管理，包括增加或移除外部数据源，改变数据清洗方法，控制出错的查询以及安排备份等。
  ...
  ![1561705895292](day09-hive讲义/1561705895292.png)	
...
![1561705918286](day09-hive讲义/1561705918286.png)	
...
![1561705940248](day09-hive讲义/1561705940248.png)	

### 2.4. Hive与传统数据库对比

hive用于海量数据的离线数据分析

![1561705959607](day09-hive讲义/1561705959607.png)	
...
将我们的hive的安装包上传到第三台(120)服务器的/export/softwares路径下，然后进行解压
...
#如果报错Could not connect to meta store using any of the URIs provided没有启动执行
...
![1561540565515](day09-hive讲义/1561540565515.png)	
...
删除一个空数据库，如果数据库下面有数据表，那么就会报错(报错也会删除)
```sql
...
1. 在hive中一共数据库就是一个文件夹,一张表也是一个文件夹,向表中插入数据时,

2. 在这些数据会生产文件数据,表文件字段直接默认的分隔符是'\001';

3. 每次执行insert插入数据,每次都会走mapreduce,都会产生一个文件;

4. create table stu3 as select * from stu2; 拷贝表时,表的内容也一起 拷贝;

5. 删除内部表,会将元数据和表数据一起删除;
...
7. 给表添加数据:
   ...
   1. 方式1:在Linux中
      ...
      hdfs dfs -put student.txt /user/hive/warehouse/mytest.db/student

   2. 方式2:在hive中

      在本地加载:表文件原来在Linux本地,加载之后,做的复制操作

      load data local inpath '/export/servers/hivedatas/students.csv' [overwrite] into table student
...
8. 分区表
   ...
   3. 给分区添加数据
      ...
      ```sql
      load data local inpath '/export/servers/hivedatas/score.csv' into table score partition (year='1999',month='06',day='23');
      ```
   ...
   4. 分区表的查询
      ...
      ```sql
      select * from score where year='1999' and month = '06' and day='01';
      ```
...
-- 使用c_id的hash值然后对整型最大值(2147483647)取&,然后对reduce个数取模
```

分桶表的数据加载，由于通标的数据加载通过hdfs  dfs  -put文件或者通过load  data均不好使，只能通过insert  overwrite
...
1. 分桶就是mapreduce中的分区,就是分文件
   ...
   1. 分桶的步骤:
      ...
      4. 给分桶表加载数据
         ...
         - 不能使用hdfs dfs -put和load local inpath...
         - 给分桶加载数据必须走mapreduce
...
1. [order by]() 会对输入做[全局排序]()，因此[只有一个reducer]()，会导致当输入规模较大时，需要较长的计算时间。
2. [sort by]()不是全局排序，其在数据进入reducer前完成排序。因此，如果用sort by进行排序，并且设置mapred.reduce.tasks>1，则[sort by只保证每个reducer的输出有序]()，不保证全局有序。
3. [distribute by]()(字段)根据指定的字段[将数据分到不同的reducer]()，且分发算法是hash散列。
4. [cluster by]()(字段) 除了具有distribute by的功能外，还会对该字段进行排序.

因此，如果[distribute 和sort字段是同一个时，此时，`cluster by = distribute by + sort by`]()
...
-- offset偏移量
select * from score limit 1 offest 1;
...
select * from score where s_score >= 80 and s_score <= 100;
```
...
- [如果你的分组字段是sid,则在select后面只能出现sid和聚合函数,不能出现其他分组字段]()
...
cluster by除了具有distribute by的功能外还兼具sort by的功能。但是[排序只能是倒序排序]()，不能指定排序规则为ASC或者DESC。
...
+ 用户自定义配置文件：[$HIVE_CONF_DIR/hive-site.xml]() (全局有效)
...
3. 编程步骤：
   1. 继承org.apache.hadoop.hive.sql.UDF
...
public class MyUDF  extends UDF{
    ...
    public Text evaluate(final Text str){//保证不能修改该字符串
...
![1565939751102](day09-hive讲义/1565939751102.png)
...
| mapreduce.map.output.compress(是否开启压缩)         | false                                                        | mapper输出  | 这个参数设为true启用压缩                     |
| mapreduce.map.output.compress.codec(设置压缩编码器) | org.apache.hadoop.io.compress.DefaultCodec                   | mapper输出  | 使用LZO、LZ4或snappy编解码器在此阶段压缩数据 |
...
**列存储的特点：** 因为每个字段的数据聚集存储，在查询只需要少数几个字段的时候，能大大减少读取的数据量；每个字段的数据类型一定是相同的，列式存储可以针对性的设计更好的设计压缩算法(kudu和hbase)。
...
![1565943001309](day09-hive讲义/1565943001309.png)
...
2）把hive.fetch.task.conversion设置成more，然后执行查询语句，如下查询方式都[不会执行mapreduce程序,走的是io流,一行一行的读取数据]()。
...
~~~sql
select count(distinct s_id) from score;#禁止使用

select count(s_id) from (select id from score group by s_id) a; #推荐使用
~~~
...
![1565947375707](day09-hive讲义/1565947375707.png)
...
Hive提供了一个严格模式，可以防止用户执行那些可能.意向不到的不好的影响的查询。

---
title: 15Hue
date: 2019/8/15 08:16:25
updated: 2019/8/15 21:52:30
comments: true
tags:
     Hue
categories: 
...
![png](ApacheHue/image2.png)
![png](ApacheHue/image3.png)
...
```shell
[root@node03 servers]# cd /export/softwares/
[root@node03 softwares]# tar  -zxvf hue-3.9.0-cdh5.14.0.tar.gz -C /export/servers/
[root@node03 softwares]# cd /export/servers/hue-3.9.0-cdh5.14.0/
```
...
# 安装ant
[root@node03 softwares]# wget http://mirrors.tuna.tsinghua.edu.cn/apache//ant/binaries/apache-ant-1.9.14-bin.tar.gz
#2、解压:
[root@centos~]# tar -xf apache-ant-1.10.1-bin.tar.gz
#3、使用ROOT权限将解压出来的文件移动到/usr/lib目录下
[root@node03 apache-ant-1.9.14]# cd /export/servers/apache-ant-1.9.14
#配置Ant环境变量
#1)、打开终端，输入[root@centos~]#/etc/profile
[root@node03 apache-ant-1.9.14]# vim /etc/profile
#2)、将刚才配置的Java环境变量修改成如下内容:
export ANT_HOME=/export/servers/apache-ant-1.9.14
export PATH=$ANT_HOME/bin:$PATH
[root@node03 apache-ant-1.9.14]# source /etc/profile
[root@node03 apache-ant-1.9.14]# ant -version
Apache Ant(TM) version 1.9.14 compiled on March 12 2019

##安装mvn
[root@node03 softwares]#  wget http://mirrors.hust.edu.cn/apache/maven/maven-3/3.1.1/binaries/apache-maven-3.1.1-bin.tar.gz

[root@node03 softwares]# tar -zxvf apache-maven-3.1.1-bin.tar.gz -C /export/servers/
[root@node03 softwares]# cd /export/servers/apache-maven-3.1.1/
#3.配置环境变量
#修改 profile 文件
##-- maven 安装的路径
vim /etc/profile

export MVEN_HOME=/export/servers/apache-maven-3.1.1/
export PATH=$MVEN_HOME/bin:$PATH
...
mvn  -v
## 报错
make[2]: *** [/export/servers/hue-3.9.0-cdh5.14.0/desktop/core/build/MySQL-python-1.2.5/egg.stamp] Error 1
make[2]: Leaving directory `/export/servers/hue-3.9.0-cdh5.14.0/desktop/core'
make[1]: *** [.recursive-env-install/core] Error 2
make[1]: Leaving directory `/export/servers/hue-3.9.0-cdh5.14.0/desktop'
# 安装mysql python
...
```shell
[root@node03 softwares]# cd /export/servers/hue-3.9.0-cdh5.14.0/desktop/conf
vim  hue.ini
```
...
```shell
[root@node03 conf]# cd /export/servers/hue-3.9.0-cdh5.14.0
[root@node03 hue-3.9.0-cdh5.14.0]# make apps
```

编译成功之后，会在hue数据库中创建许多初始化表。
![png](ApacheHue/image5.png)
![png](ApacheHue/image6.png)

## 启动Hue、Web UI访问

```shell
[root@node03 hue-3.9.0-cdh5.14.0]# useradd hue
[root@node03 hue-3.9.0-cdh5.14.0]# passwd hue
...
![png](ApacheHue/image10.png)
![png](ApacheHue/image11.png)
...
![png](ApacheHue/image13.png)

### 利用hue调度shell脚本

在HDFS上创建一个shell脚本程序文件。
![png](ApacheHue/image14.png)
![png](ApacheHue/image15.png)
![png](ApacheHue/image16.png)
打开工作流调度页面。
![png](ApacheHue/image17.png)
![png](ApacheHue/image18.png)
![png](ApacheHue/image19.png)
![png](ApacheHue/image20.png)
![png](ApacheHue/image21.png)
![png](ApacheHue/image22.png)

### 利用hue调度hive脚本

在HDFS上创建一个hive sql脚本程序文件。
![png](ApacheHue/image23.png)

```sql
use myhive;
create table stue(id int,name string);
insert into stue values (1,"zhangsan");
insert into stue values (2,"zhangsan2");
```

打开workflow页面，拖拽hive2图标到指定位置。
![png](ApacheHue/image24.png)
![png](ApacheHue/image25.png)
![png](ApacheHue/image26.png)
![png](ApacheHue/image27.png)
![png](ApacheHue/image28.png)

### 利用hue调度MapReduce程序

利用hue提交MapReduce程序
![png](ApacheHue/image29.png)
![png](ApacheHue/image30.png)
![png](ApacheHue/image31.png)
![png](ApacheHue/image32.png)

### 利用Hue配置定时调度任务

在hue中，也可以针对workflow配置定时调度任务，具体操作如下：
![png](ApacheHue/image33.png)
![png](ApacheHue/image34.png)
![png](ApacheHue/image35.png)
![png](ApacheHue/image36.png)
一定要注意时区的问题，否则调度就出错了。保存之后就可以提交定时任务。
![png](ApacheHue/image37.png)
![png](ApacheHue/image38.png)
点击进去，可以看到定时任务的详细信息。
![png](ApacheHue/image39.png)
![png](ApacheHue/image40.png)
...
![png](ApacheHue/image41.png)
![png](ApacheHue/image42.png)
![png](ApacheHue/image43.png)
...
![png](ApacheHue/image44.png)
![png](ApacheHue/image45.png)

## 附件

### mapreduce_oozie_workflow.xml
